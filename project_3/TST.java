...
public class TST<Value> {
    ...
    **************************************************************/
    ...
    public Iterable<StringBuilder> wildcardMatch(StringBuilder pat) {
        Queue<StringBuilder> queue = new Queue<StringBuilder>();
        StringBuilder arg = new StringBuilder();
        collect(root, arg, 0, pat, queue);
        return queue;
    }
 
    public void collect(Node x, StringBuilder prefix, int i, StringBuilder pat, Queue<StringBuilder> q) {
        if (x == null) return;
        char c = pat.charAt(i);
        if (c == '.' || c < x.c) collect(x.left, prefix, i, pat, q);
        if (c == '.' || c == x.c) {
            if (i == pat.length() - 1 && x.val != null) q.enqueue(prefix.append(x.c));
            if (i < pat.length() - 1) collect(x.mid, prefix.append(x.c), i+1, pat, q);
        }
